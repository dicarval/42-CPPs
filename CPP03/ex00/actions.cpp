...
/*   Updated: 2025/06/30 15:56:43 by dicarval         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ClapTrap::takeDamage(unsigned int amount)
{
	if (_hitPoints == 0)
	{
		std::cout << "ClapTrap " << _name << " has 0 hit points, it's not possible to"\
...
void	ClapTrap::attack(const std::string& target)
{
	if (_hitPoints == 0)
	{
		std::cout << "ClapTrap " << _name << " has already 0 hit points, it's not possible to attack"\
		 << std::endl;
		return ;
	}
	if (_energyPoints == 0)
